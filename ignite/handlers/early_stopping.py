...
class EarlyStopping(object):
    ...
    def __init__(self, patience, score_function, trainer):
        ...
        self.valid_run_num = 0
        self.best_state = {'metrics': {}, 'epoch': 0, 'iters': 0, 'valid_iters': 0}

        self._logger = logging.getLogger(__name__ + "." + self.__class__.__name__)
        self._logger.addHandler(logging.NullHandler())

    def __call__(self, engine):
        self.valid_run_num += 1

        score = self.score_function(engine)

        if self.best_score is None:
            self.best_score = score
            self.best_state['epoch'] = self.trainer.state.epoch
            self.best_state['iters'] = self.trainer.state.iteration
            self.best_state['valid_iters'] = self.valid_run_num
            self.best_state['metrics'] = deepcopy(engine.state.metrics)
